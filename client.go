...
// ZoneEntities представляет отдельный DNS
...
type SecondaryDNS struct {
	Entities []ZoneEntities `json:"entities"`
}
...
type Zone struct {
	...
	SecondaryDns    SecondaryDNS `json:"secondary_dns,omitempty"`
	SOAEmail        string       `json:"soa_email,omitempty"`
	TTL             int          `json:"ttl,omitempty"`
	UpdatedAt       string       `json:"updated_at,omitempty"`
}

// Records представляет список записей DNS
type Records struct {
	Count    int      `json:"count,omitempty"`
	Entities []Record `json:"entities,omitempty"`
}
...
var recordTypes = [...]string{"A", "AAAA", "CNAME", "MX", "NS", "TXT", "SRV", "CAA"}
...
// 	Count    int         `json:"count"`
// 	Entities RecordsList `json:"getrecords"`
// }
...
func GetZoneID(zone_name string, c *NetangelsClient) (int, error) {
	...
	z_name := domainutil.Domain(zone_name)
	var zoneID int
...
func (c *NetangelsClient) AddRecord(FQDNName string, Value string, recordType string, ttl int) (int, error) {
...
func (c *NetangelsClient) GetRecordID(FQDNName string, RecordData string, recordType RecordType) (int, error) {
	...
	for _, item := range responseData.Entities {
		if item.Type == recordType {
			if item.Details.Value == RecordData {
				record_id = item.ID
...
func (c *NetangelsClient) GetRecord(FQDNName string, RecordData string, recordType RecordType) (int, string, error) {
	...
	for _, item := range responseData.Entities {
		if item.Type == recordType {
			if item.Name == fqdnName {
				if RecordData == "" {
					return item.ID, item.Details.Value, nil
				} else {
					if item.Details.Value == RecordData {
						return item.ID, item.Details.Value, nil
					}
...
// GetRecords Fetch records by FQDNName in struct Records
func getRecords(FQDNName string, c *NetangelsClient) (Records, error) {
	fqdnName := cutTrailingDotIfExist(FQDNName)
	...
	if err != nil {
		return Records{}, fmt.Errorf("ошибка создания запроса: %v", err)
	}
	// Установка заголовка авторизации
	req.Header.Set("Authorization", "Bearer "+c.ApiToken)
	client := &http.Client{}
	...
	// Декодирование JSON-ответа в структуру Zones
	var record_list Records
	if err := json.Unmarshal(responseData, &record_list); err != nil {
		return Records{}, fmt.Errorf("ошибка декодирования JSON: %v", err)
	}
...
func (c *NetangelsClient) UpdateRecord(ID int, FQDNName string, Value string, recordType string, Ttl int) (bool, error) {
	if !validateRecordType(recordType) {
		log.Errorln("Invalid record type: ", recordType)
	...
	req, err := http.NewRequest("POST", apiUrl+"/dns/records/"+strconv.Itoa(ID)+"/", bytes.NewBuffer(putBody))
	if err != nil {
...
// // DDNS update/create record if Ip omited Netangels api will use client IP
